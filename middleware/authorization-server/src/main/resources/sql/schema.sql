...
CREATE TABLE IF NOT EXISTS public.user_authority (
    ...
    FOREIGN KEY (username) REFERENCES public.tb_user (username),
    FOREIGN KEY (authority) REFERENCES public.authority (name)
);

CREATE UNIQUE INDEX user_authority_idx_1 ON public.user_authority (username, authority);
